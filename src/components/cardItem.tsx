...
export default function CardItem({ product, onDelete }: { product: ProductData, onDelete: Function }) {
  ...
  const renderStatusBadge = (status: string, text: string, color: string,) => {
    return (
      product.status === status && (
        <Badge variant="outline" className={`px-1.5 mx-0.5 ${color} mb-3`}>
          {text}
        </Badge>
      )
    );
    ...
    return (
      <div>
        {product.sizes.map((size: Size) => (
          <Badge key={size.code} variant="outline" className="px-1.5 mx-0.5 bg-gray-50">
            {size.code}
          </Badge>
        ))}
      </div>
  ...
  return (
    <Card className="flex flex-col justify-between">
      ...
      <CardContent className="gap 4 pl-5 pr-5 pb-2">
        <div>
          {renderStatusBadge('ACTIVE', 'Active', 'bg-green-100')}
          {renderStatusBadge('HOLD', 'Hold','bg-red-100')}
          {renderStatusBadge('CLOSEOUT', 'Closeout', 'bg-amber-100')}
